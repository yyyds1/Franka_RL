...
def ball_ground_contact_penalty(
    env: ManagerBasedRLEnv, 
    ball_cfg: SceneEntityCfg = SceneEntityCfg("ball"),
    threshold: float = 0.5
) -> torch.Tensor:
    """Penalize when the ball touches the ground.
    
    Args:
        env: The environment.
        ball_cfg: Configuration for the ball asset.
        threshold: Height threshold below which ball is considered to be on ground.
    
    Returns:
        Penalty value (positive when ball is on ground).
    """
    # Get ball asset
    ball: RigidObject = env.scene[ball_cfg.name]
    
    # Get ball height (z position)
    ball_height = ball.data.root_pos_w[:, 2]
    
    # Ball radius (assumed to be 0.06m based on your configuration)
    ball_radius = 0.06
    
    # Check if ball is on ground (center height less than radius + threshold)
    on_ground = ball_height < (ball_radius + threshold)
    
    return on_ground.float()


def ball_robot_distance_reward(
    env: ManagerBasedRLEnv,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    ball_cfg: SceneEntityCfg = SceneEntityCfg("ball"),
    max_distance: float = 2.0
) -> torch.Tensor:
    """Reward based on proximity between robot and ball.
    ...
    Returns:
        Reward value (higher when robot is closer to ball).
    """
    ...
    # Get positions
    robot_pos = robot.data.root_pos_w[:, :2]  # Only x, y coordinates
    ball_pos = ball.data.root_pos_w[:, :2]    # Only x, y coordinates
...
def ball_carrying_reward(
    env: ManagerBasedRLEnv,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    ball_cfg: SceneEntityCfg = SceneEntityCfg("ball"),
    carry_height: float = 0.3,
    carry_distance: float = 0.5
) -> torch.Tensor:
    """Reward for successfully carrying the ball (ball close to robot and above ground).
    
    Args:
        env: The environment.
        robot_cfg: Configuration for the robot asset.
        ball_cfg: Configuration for the ball asset.
        carry_height: Minimum height for ball to be considered "carried".
        carry_distance: Maximum horizontal distance for ball to be considered "carried".
    
    Returns:
        Reward value (positive when ball is being carried).
    """
    # Get robot and ball assets
    robot: Articulation = env.scene[robot_cfg.name]
    ball: RigidObject = env.scene[ball_cfg.name]
    ...
    horizontal_distance = torch.norm(ball_pos[:, :2] - robot_pos[:, :2], dim=1)
    close_horizontally = horizontal_distance < carry_distance
    
    # Check height
    ball_radius = 0.06
    ball_above_ground = ball_pos[:, 2] > (ball_radius + carry_height)
    
    # Combined carrying condition
    carrying = close_horizontally & ball_above_ground
    
    return carrying.float()
